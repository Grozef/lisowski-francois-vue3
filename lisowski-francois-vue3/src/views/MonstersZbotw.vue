...
import MonstreGeneral from '../components/MonstreGeneral.vue';

const API_ENDPOINT = 'https://metallo.ew.r.appspot.com/monsters'

const zeldaMonsters = ref([])
const isLoaded = ref(false)

const router = useRouter()

onBeforeMount(async ()=>{
  const zeldaMonsters = await axios.get(API_ENDPOINT)
  const {data, status} = zeldaMonsters
  if(status==200){
    zeldaMonsters.value = data
    isLoaded.value = true
    console.log(data)
  }else{
    console.log('OUPS :(')
...
<section data-bs-version="5.1" class="features05 cid-sRQcud4Jjb" id="features05-2">
    ...
    <div class="container-fluid">
        <div class="wrapper">
            ...
            v-for="monster in zeldaMonsters" :key="monster._id">
